...
public class ItemManager : MonoBehaviour
{
    ...
    public ItemSlotScript selectedItemSlotScript;

    public ItemScript selectedItemScript;
    ...
    void SelectHotbar()
    {
        for (int i = 0; i < items.Length; i++)
            ...
            {
                if (itemSlots[i] == selectedItemSlotScript)
                {
                    selectedItemSlotScript.Deselect();
                    return;
                }
                if (selectedItemSlotScript)
                {
                    selectedItemSlotScript.Deselect();
                }
                itemSlots[i].Select();
    ...
    // Adds a item to the inventory in the first open slot (Adjusts transform values)
    // If a Stackable item, it will add to the count of the same stackable item if it's in the inventory
    ...
    public void AddItem(GameObject itemToAdd)
    {
        ...
        itemToAdd.GetComponent<SphereCollider>().enabled = false;

        if (itemToAdd.GetComponent<ItemScript>().itemData.itemType == ItemData.ItemType.STACKABLE)
        {
            foreach (ItemScript item in items)
            {
                // This feels really redundant checking the type in a foreach but it has to be I guess T-T
                if (item is ItemScript && itemToAdd.GetComponent<ItemScript>().itemData.itemName == item.itemData.itemName)
                {
                    Destroy(itemToAdd);
                    item.count += itemToAdd.GetComponent<ItemScript>().count;
                    return;
                }
            }
        }
        // Check each item slot and add to the first empty one
        foreach (ItemSlotScript itemSlot in itemSlots)
        {
            if (itemSlot.GetComponentInChildren<ItemScript>() == null)
            {
                itemToAdd.transform.SetParent(itemSlot.transform, false);
    ...
    {
        // Rigidbody rb = selectedItemScript.GetComponent<Rigidbody>();
        // float magnitude = 10;
        // rb.useGravity = true;
        // selectedItemScript.transform.SetParent(null);
        // selectedItemScript.transform.position = PlayerScript.instance.gameObject.transform.position;
        // selectedItemScript.transform.localScale = new Vector3(10, 10, 10);
        // rb.AddForce(Vector3.forward * magnitude, ForceMode.Impulse);

//selectedItemScript.transform.rotation = camera.transform.rotation;
//selectedItemScript.transform.eulerAngles = new Vector3(0, selectedItemScript.transform.rotation.eulerAngles.y, selectedItemScript.transform.rotation.eulerAngles.z);       
//selectedItemScript.transform.Translate(transform.forward * 5);
//selectedItemScript.GetComponent<SphereCollider>().enabled = true;
        // selectedItemSlotScript.Deselect();
        // GetItems();
    }
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown("q") && selectedItemScript)
        {
            DropItem();
        }
    }
